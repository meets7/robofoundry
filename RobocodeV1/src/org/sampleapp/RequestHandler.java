...
public class RequestHandler extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response){
		...
		AppParameter.setTenantDomainName(request.getParameter("tenant"));
		String listofTenants = this.getServletConfig().getInitParameter("TenantDomainName");
		String[] tenantsList = listofTenants.split(",");
		index = tenantsList.toString().indexOf(request.getParameter("tenant"));
		System.out.println(index);
		for (int i=0;i<tenantsList.length;i++)
		{
			if (tenantsList[i].toString().equals(request.getParameter("tenant")))
			{
				index = i;
			}
		}
		String[] appprincipalList = this.getServletConfig().getInitParameter("AppPrincipalId").split(",");
		String[] symmetricKeyList = this.getServletConfig().getInitParameter("SymmetricKey").split(",");
		System.out.println(appprincipalList[index]);
		AppParameter.setAppPrincipalId(appprincipalList[index]);
		AppParameter.setSymmetricKey(symmetricKeyList[index]);
		...
		while(retryRemaining-- > 0){
			try{
				...
				case "createUser":
				    ...
				    System.out.println("SQL" + sqltocheck);
				    while(rl.next())
				    {
				    	countUsers = Integer.parseInt(rl.getString("countUsers"));
				    }
				    System.out.println("Count" + countUsers);
					if (request.getParameter("userAction").toString().equals("UserCreation") && countUsers > 0) 
					{
						System.out.println("Inside if");
						request.getSession().setAttribute("message","User name already exists in some other tenant. Use User replication instead if you are trying to give a user access to multiple tenants");
